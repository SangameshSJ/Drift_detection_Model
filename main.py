...
def main():
    ...
    for i in res.keys():
        for j in reference_data.columns:
            if j != 'Date':
                if i == "mean_variance_shift":
                    print('Meanshift',j,res[i][j]['Mean Shift'])
                    print('varience',j,res[i][j]['Variance Shift'])
                else:
                    print(i, j, res[i][j])
    generate_drift_report(reference_data, current_data, drift_results,res)
